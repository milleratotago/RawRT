...
[~,EndingVals,Best,exitflag] = DistObj.EstML(inDVs);
...
[~,EndingVals,Best,exitflag] = DistObj.EstMom(ObsMoments);
out = [DistObj.ParmValues Best exitflag];
if ~UseFn
    DistObj.ResetParms(HoldParms);   % Restore original parameter values
end
...
[~,EndingVals,Best,exitflag] = DistObj.EstChiSq(BinMax,BinProbs);
out = [DistObj.ParmValues Best exitflag];
if ~UseFn
    DistObj.ResetParms(HoldParms);   % Restore original parameter values
end
...
ObsPctiles = prctile(inDVs,TargetCDFs*100);
[~,EndingVals,Best,exitflag] = DistObj.EstPctile(ObsPctiles,TargetCDFs);
out = [DistObj.ParmValues Best exitflag];
if ~UseFn
    DistObj.ResetParms(HoldParms);   % Restore original parameter values
end
